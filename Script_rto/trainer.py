...
class ChestXrayHandler:
    def __init__(self, args):
        ...
        self.img_map = create_dataset_dict(args.csv_path,key_col= "Image Index",val_col= "path")
    ...
    def split_data(self, csv_path):
        ...
        print(test["Image Index"].head(10))
    ...
    def build_model(self):
        ...
        # register attention hooks on all transformer blocks
        #for blk in self.model.vit.blocks:
            #blk.attn.attn_drop.register_forward_hook(self.model.vit_attn_hook)
        self.model.register_vit_hooks()
    ...
    def generate_vit_map(self,batch= None,img_name = str, batch_index = 0, save_dir = None):
        #self.checkpoint = torch.load("best_path", map_location = self.device)
        ...
        #Extract attention for chosen batch item
        #attn_map = vit_attn_map(self.model.attn_maps, batch_index)

        #Retrieve the image path from self.im_map dictionary
        #img_name = img_names[batch_index]
        img_path = self.img_map[img_name]
        print(img_path)
        #Load original image for overlay plot
        full_imgpath = os.path.join(img_path,img_name)
        img = Image.open(full_imgpath).convert("RGB")
        ...
        #Extract attention for chosen image
        attn_map = vit_attn_map(self.model.attn_maps, batch_index)
        #save_path = f"{save_dir}/attn_{img_name}"
        save_path = os.path.join(save_dir,"attn_map")
        plot_attn_map(img,img_name,attn_map, save_path)
        print(f"Saved attention map to {save_dir}")
        #img = Image.open(img_path).convert("RGB")
        #attn_map = vit_attn_map(self.model.attn_stored)
        #attn_map = cv2.resize(attn_map, (224,224))
        #attn_heatmap = cv2.applyColorMap(np.uint8(attn_map * 255), cv2.COLORMAP_JET)
        #return attn_heatmap
