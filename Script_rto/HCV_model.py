...
class DenseNetViT(nn.Module):
    def __init__(self, num_classes=14, freeze_backbone=False, freeze_vit=False, use_custom_proj=False, train_last_vit_layers=None, custom_proj_layers=512):
        ...
        # Add Hooks to capture vit attention maps
        self.attn_maps = []
        #for block in self.vit.encoder.layers:
            #block.attn.attn_drop.register_forward_hook(self.vit_attn_hook)
    ...
    # Hook function to capture attention from each transformer block
    def vit_attn_hook(self, module, input, output):
        self.attn_maps.append(output.detach())

    def register_vit_hooks(self):
        for blk in self.vit.blocks:
            blk.attn.fused_attn = False
            blk.attn.attn_drop.register_forward_hook(self.vit_attn_hook)
    
    
    def forward(self, images):
        self.attn_maps = []  #clear previously stored maps
